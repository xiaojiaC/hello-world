...
从方法名称派生查询并不总是合适的，并且可能导致不可读的方法名称。在这种情况下，你可以使用Solr命名查询（请参阅“<<solr.query-methods.named-queries,使用命名查询>>”）或使用 `@Query` 注解（请参阅“<<solr.query-methods.at-query,使用@Query注解>>”）。
...
命名查询可以保存在属性文件中并连接到相应的方法。你应该记住“<<repositories.query-methods.query-lookup-strategies,查询查询策略>>”中描述的命名约定或使用 `@Query`。以下示例显示如何在属性文件中声明命名查询：
...
使用 `SolrRepository` 无法直接应用Faceting，但 `SolrTemplate` 支持此功能。以下示例显示了一个分面查询：
...
字段或查询的Facets也可以使用 `@Facet` 定义。请记住，结果是 `FacetPage`。

NOTE: 使用 `@Facet` 可以定义使用输入参数作为值的占位符。

以下示例使用 `@Facet` 注解定义Facet查询：
...
Solr允许基于每个字段定义facet参数。要向定义的字段添加特殊facet选项，请使用 `FieldWithFacetParameters`，如以下示例所示：
...
你可以通过在 `FacetOptions` 上配置所需范围来创建范围Faceting查询。你可以通过创建 `FacetOptions` 实例，将选项设置为 `FacetQuery`，并通过 `SolrTemplate` 查询Facet页来请求范围，如下所示：
...
还支持Pivot faceting（决策树），可以使用 `@Facet` 注解查询，如下所示：
...
术语向量不能直接在 `SolrRepository` 中使用，但可以通过 `SolrTemplate` 应用。请记住，结果是 `TermsPage`。以下示例显示如何创建术语查询：
...
要突出显示搜索结果中的匹配项，可以将 `HighlightOptions` 添加到 `SimpleHighlightQuery`。提供没有任何其他属性的 `HighlightOptions` 会在 `SolrDocument` 中的所有字段上突出显示。
...
要将突出显示应用于派生查询，可以使用 `@Highlight`。如果没有定义 `fields`，则突出显示应用于所有字段。
