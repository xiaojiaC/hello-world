[[testing]]
= 测试
[.small]#<<spring-webflux.adoc#webflux-test,与Spring WebFlux相同>>#
...
* Client-side REST: `spring-test` 提供了一个 `MockRestServiceServer`，你可以将其用作模拟服务器，以测试内部使用 `RestTemplate` 的客户端代码。
